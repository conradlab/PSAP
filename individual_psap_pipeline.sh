...
PSAP_PATH=/home/users/conradon/clab/PSAP/
ANNOVAR_PATH=/home/users/conradon/clab/annovar/
FORCE="FALSE"
#FORCE="-F"
...
# RUN PSAP_individual.R for each individual
	...
	for i in ${IDS[@]}
	do	
		if [ ! -f avinput/$i.avinput ]
                   then
                   bcftools view $VCF -s $i -c 1 | bcftools annotate -x ^INFO/AC >  $i.tmp.vcf 
                  perl ${ANNOVAR_PATH}convert2annovar.pl -format vcf4old $i.tmp.vcf -outfile avinput/$i.avinput -includeinfo
		fi
		Rscript ${PSAP_PATH}RScripts/PSAP_individual.R ${OUTFILE} $i $PSAP_PATH $PED_FILE $HG_BUILD $FORCE $INDEL_FILE
#		rm $i.tmp.vcf
