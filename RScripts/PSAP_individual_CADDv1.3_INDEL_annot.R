...
#arg<-c("wuwu","H_VZ-Copen-56-Copen-56_clean_b37d.variant2641","/scratch/dclab/psap4/psap/","/scratch/dclab/gurnett/wuwu_gemini.ped","hg19","FALSE")
...
indel.scores = read.table("/scratch/dclab/psap4/wulin_WES_6169_INDELs_CADDv1.3.tsv", sep='\t', quote='', header=T, comment.char='')
indel.scores$REF <- as.character(indel.scores$REF)
indel.scores$ALT <- as.character(indel.scores$ALT)
# Reformat to match annovar's annotation of INDELs (from 0-based to 1-based)
...
indel.scores$Key<-paste(indel.scores$X.CHROM,":",indel.scores$POS,"_",indel.scores$REF,">",indel.scores$ALT,sep="")
  
drops <- c("CADD13_RawScore","CADD13_PHRED")
indels = exome[grep("shift",exome[,loc_col]), !names(exome) %in% drops]
indels = merge(indels, indel.scores, by.x=1, by.y=7, all.x=T, all.y=F)

names(indels)[names(indels)=="RawScore"] <- "CADD13_RawScore"
names(indels)[names(indels)=="PHRED"] <- score
indels <- indels[names(exome)]
exome <- rbind(exome[-grep("shift",exome[,loc_col]),], indels)
